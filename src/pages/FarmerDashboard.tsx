...
export default function FarmerDashboard() {
  ...
  const statusDesc: Record<string, string> = {
    ...
    harvesting: '🌿 Agent is currently harvesting',
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <div className="flex items-center gap-2 mb-5 flex-wrap">
          ...
          {roles.includes('agent') && (
            <button
              ...
              className="bg-blue-500 bg-opacity-90 text-white text-xs px-3 py-1.5 rounded-full font-bold">
              🧑‍🌾 Switch to Agent
            </button>
          )}
        </div>
      ...
      <div className="px-4 -mt-16 space-y-4">
        ...
        <div className="bg-white rounded-3xl shadow-sm p-5">
          ...
          <div className="grid grid-cols-4 gap-3">
            <QuickActionBtn emoji="📅" label="Schedule" color="bg-green-50 border-green-200" onClick={() => navigate('/farmer/harvest/new')} />
            <QuickActionBtn emoji="🌿" label="Harvests" color="bg-teal-50 border-teal-200" onClick={() => setActiveTab('harvests')} />
            <QuickActionBtn emoji="🌱" label="AgroVet" color="bg-emerald-50 border-emerald-200" onClick={() => navigate('/farmer/agrovet')} />
            <QuickActionBtn emoji="💰" label="Loans" color="bg-purple-50 border-purple-200" onClick={() => setActiveTab('loans')} />
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm p-5">
          ...
          {prices.length === 0 ? (
            <div className="bg-gray-50 rounded-2xl p-5 text-center">
              ...
              <p className="text-gray-500 text-sm font-medium">Prices not set yet today</p>
              ...
              {recentHarvests.length === 0 ? (
                <div className="bg-green-50 rounded-2xl p-6 text-center">
                  ...
                  <p className="text-gray-400 text-xs mt-1 mb-4">
                    Request your first harvest agent
                  </p>
                  <button onClick={() => navigate('/farmer/harvest/new')}
                      ...
                      <div key={h.id} className="border border-gray-100 rounded-2xl overflow-hidden">
                        ...
                        <div className="px-4 py-3">
                          ...
                          <div className="w-full bg-gray-100 rounded-full h-2 mb-2">
                            <div className={`h-2 rounded-full ${progressColor[h.status] || 'bg-gray-300'} ${progressWidth[h.status] || 'w-0'}`} />
                          </div>
                          ...
                          {h.agentName && (
                            <p className="text-xs text-blue-600 mt-1">🧑‍🌾 Agent: {h.agentName}</p>
                          )}
                          {h.actualWeightKg && (
                            <p className="text-xs text-gray-600 mt-1">
                              ⚖️ Actual: {h.actualWeightKg} kg
                              {h.estimatedWeightKg && (
                                <span className={`ml-1 font-bold ${
                                  h.actualWeightKg >= h.estimatedWeightKg
                                    ? 'text-green-600' : 'text-red-500'
                                }`}>
                                  ({h.actualWeightKg >= h.estimatedWeightKg ? '+' : ''}
                                  {(h.actualWeightKg - h.estimatedWeightKg).toFixed(1)} kg from estimate)
                                </span>
                              )}
                            </p>
                          )}
            ...
            <button onClick={() => navigate('/farmer/agrovet')}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  ...
                  <p className="text-gray-500 text-xs mt-0.5">Order farm inputs & supplies</p>
                </div>
              </div>
                      ...
                      <div className="p-4 space-y-2">
                        ...
                        {h.agentName && (
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium text-blue-600">{h.agentName}</span>
                          </div>
                        )}
                        {/* Weight comparison */}
                              ...
                              <div>
                                ...
                                <p className={`font-bold text-sm ${
                                  ...
                                  {h.weightVarianceKg != null
                                    ? `${h.weightVarianceKg >= 0 ? '+' : ''}${Number(h.weightVarianceKg).toFixed(1)} kg`
                                    : '—'
                                  }
                                </p>
                              </div>
        ...
        {activeTab === 'savings' && (
          <div className="space-y-4">
            <div className="bg-white rounded-3xl shadow-sm p-5">
              ...
              {savingsAccounts.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-400 text-xs mt-1 mb-4">
                    Visit SACCO office to open an account
                  </p>
                  <a href="tel:+254757630995"
                    className="inline-block bg-green-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold">
                    📞 Call Office
                  </a>
                </div>
              ) : (
                  ...
                  {savingsAccounts.map((acc: any) => (
                    <div key={acc.id}
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-4">
                        <button onClick={() => navigate('/farmer/mpesa/deposit')}
                          className="bg-white bg-opacity-20 text-white text-sm font-bold py-2 rounded-xl border border-white border-opacity-30 active:bg-opacity-30">
                          📥 Deposit
                        </button>
                        <button onClick={() => navigate('/farmer/mpesa/withdraw')}
                          className="bg-white bg-opacity-20 text-white text-sm font-bold py-2 rounded-xl border border-white border-opacity-30 active:bg-opacity-30">
            ...
            <div className="bg-white rounded-3xl shadow-sm p-5">
              <p className="font-black text-gray-900 mb-4">💡 How to Manage Savings</p>
              <div className="space-y-3">
                {[
                  { n: '📥', t: 'Deposit via M-Pesa', d: 'Tap Deposit above — M-Pesa prompt sent to your phone' },
                  { n: '💸', t: 'Withdraw to M-Pesa', d: 'Tap Withdraw — money sent directly to your phone' },
                  { n: '📊', t: 'Balance Updates Live', d: 'Your balance reflects every transaction instantly' },
                ].map((item, i) => (
        ...
        {activeTab === 'loans' && (
          <div className="space-y-4">
            {activeLoans.length > 0 ? (
                      ...
                      <div key={loan.id} className="border border-gray-200 rounded-2xl overflow-hidden">
                        ...
                        <div className="p-4">
                          ...
                          <button onClick={() => navigate('/farmer/mpesa/repay')}
                            className="w-full bg-green-600 text-white text-sm font-bold py-3 rounded-xl flex items-center justify-center gap-2">
                            📱 Repay via M-Pesa
                          </button>
                        </div>
                      </div>
              ...
              <div className="bg-white rounded-3xl shadow-sm p-5 text-center py-8">
                <p className="text-5xl mb-3">💰</p>
            ...
            <div className="bg-white rounded-3xl shadow-sm p-5">
              ...
              <div className="space-y-3 mb-4">
                {[
                  ...
                  { n: 2, t: 'Bring Documents', d: 'National ID, Member No., guarantor details' },
              ...
              <div className="bg-green-50 border border-green-200 rounded-2xl p-4 mb-3">
                ...
                <ul className="text-xs text-green-700 space-y-1">
                  <li>• Active SACCO member for 3+ months</li>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 py-2 shadow-lg">
        <div className="flex items-center justify-around max-w-lg mx-auto">
          <BottomNavBtn emoji="🏠" label="Home" active={activeTab === 'home'} onClick={() => setActiveTab('home')} />
          <BottomNavBtn emoji="🌿" label="Harvests" active={activeTab === 'harvests'} onClick={() => setActiveTab('harvests')} />
          ...
          <BottomNavBtn emoji="💵" label="Savings" active={activeTab === 'savings'} onClick={() => setActiveTab('savings')} />
          <BottomNavBtn emoji="👤" label="Profile" active={false} onClick={() => navigate('/farmer/profile')} />
        </div>
      </div>
    </div>
  )
